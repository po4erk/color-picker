...
const DropdownSlider = ({updateColor,makeHex,cancelColor,red,green,blue}) => {
    ...
    const rgbInputs = rgb.map(({name, id, color}) => 
        <div className="input_range" key={id}>
            <label htmlFor={rgb.id} className="input_range">{name}</label>
            <input id={id} className="input_range" type="range" min="0" max="255" steps="1" value={color}
    ...
    return(
        ...
        <div className='dropdown_color' style={style}></div>
            {rgbInputs}
            <button className="btnOk" onClick={makeHex}>OK</button>
